...
/* exported Particles */
let Particles = (function (window, document) {
  "use strict";
  ...
  Plugin.prototype._initializeStorage = function () {
    ...
    for (let i = _.options.maxParticles; i--; ) {
  ...
  /**
   * Kick off letious things on window resize.
   *
   * @private
   */
  ...
  Plugin.prototype._drawEdge = function (p1, p2, opacity) {
    ...
    let color1 = this._hex2rgb(p1.color);
    let color2 = this._hex2rgb(p2.color);
  ...
  Particle = function (context, options) {
    ...
    let canvas = document.querySelector(options.selector);
    ...
    _.radius = random() * random() * options.sizeletiations;
    _.color = color;
